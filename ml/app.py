...
class Config:
    MODEL_PATH = "."  
    MODEL_FILE = "indian_carbon_model_xgboost.pkl"
    MODEL_INFO_FILE = "indian_model_info.pkl"
...
try:
    ...
    predictor = None
    print(f"FATAL: Application will not run due to model loading error: {e}")
...
def home():
    """Root endpoint"""
    return jsonify({
        "message": "Carbon Emission Predictor API",
        "version": "1.0",
        "endpoints": {
            "/api/metrics": "GET - Model performance metrics",
            "/api/projections": "GET - CO2 predictions (params: latitude, longitude, date)"
        }
    })
...
def get_projections():
    """Endpoint for future projections."""
    if predictor is None:
        return jsonify({"error": "Model not loaded"}), 500

    try:
        latitude = float(request.args.get('latitude', 28.7041))
        longitude = float(request.args.get('longitude', 77.1025))
        date = request.args.get('date', datetime.now().strftime('%Y-%m-%d'))
        
        # Validate date format
        datetime.strptime(date, '%Y-%m-%d')
        
        projection_data = predictor.predict_co2(latitude, longitude, date)
